...
def main():
    ...
    init_image, input_image = process_img(args.input_image, args.resolution)
    ...
    for lr in learning_rates:
        for n_hiper in n_hipers:
            ...
            def record_result(hiper_embeddings, epoch, loss, seed_fix=False):
                file_path = f'{args.output_dir}/result.txt'
                PSNR_list = []
                PSNR1_list = []
                lpips_list = []
                for i in range(5):
                    ...
                    with open(file_path, 'a') as file:
                        file.write("\n" + f"---{i}th image lpips : {lpips}, PSNR : {PSNR} , PSNR1 : {PSNR1}")
                
                    save_name = os.path.join(args.output_dir, '{}_n_hiper{}_epoch_{}_lr{}_i.png'.format(args.source_text, n_hiper, epoch, lr, i))
                ...
                embedding_name = os.path.join(args.output_dir, f'{args.source_text}_{epoch}_nhiper{n_hiper}.pt')
                torch.save(hiper_embeddings.cpu(), embedding_name)

                # save quantative results
                lab_text = f"dataset_{args.source_text}, epoch_{epoch}, n_hiper_{n_hiper}, learning_rate_{optimizer.param_groups[0]['lr']} loss_{loss}"

                train_history['lpips'].append(np.mean(lpips_list))
                train_history['psnr'].append(np.mean(PSNR1_list))
                with open(file_path, 'a') as file:
                    file.write("\n" + str(datetime.now()) + lab_text)
                    file.write("\n" + f"len : {len(lpips_list)} , lpips : {np.mean(lpips_list)}, PSNR : {np.mean(PSNR_list)} , PSNR1 : {np.mean(PSNR1_list)} \n")
                
                # fig = plt.figure()
                # fig.plot(train_history['lpips'])
                # fig.savefig(args.output_dir+'/lpips.png')

                # fig2 = plt.figure()
                # fig2.plot(train_history['psnr'])
                # fig2.savefig(args.output_dir+'/psnr.png')
